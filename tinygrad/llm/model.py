...
_debug_first_layer = False

def resolve_symbolic(t: Tensor|None, **var_vals: int):
  # make a symbolic tensor concrete and return its numpy array:
  # 1. strip BIND nodes, collecting their runtime values  2. replace remaining DEFINE_VARs with CONSTs
  from tinygrad.helpers import all_int, Context
  from tinygrad.engine.realize import capturing
  if t is None: return None
  if capturing: return "<JIT capturing, can't access tensor data>"
  # ALLOW_DEVICE_USAGE=1: @function(precompile=True) disables kernel execution during tracing, re-enable it for this debug print
  # .numpy() must also happen inside this context since it realizes the CPU copy
  with Context(DEBUG=0, ALLOW_DEVICE_USAGE=1):
    if all_int(t.shape): return t.numpy()
    uop, bound = t.uop.unbind_all()
    vals = var_vals | {v.arg[0]: val for v, val in bound.items()}  # bound runtime values win over kwargs
    subs = {v: v.const_like(vals[v.arg[0]]) for v in uop.variables() if v.arg[0] in vals}
    return Tensor(uop.substitute(subs)).numpy()

@functools.cache
def precompute_positions(end:int, device:str|None=None) -> Tensor:
...
class FFNBlock:
  ...
  def _attention(self, x:Tensor, start_pos:int|UOp, pad_lens:Tensor) -> Tensor: raise NotImplementedError

  def __call__(self, x: Tensor, start_pos: int|UOp, pad_lens:Tensor):
    self._init_state(x)
    # we pass in the weights implicitly so we unpack the GGUF on the fly
    @function(precompile=True, allow_implicit=True)
    def _run(x:Tensor, start_pos:int|UOp, pad_lens:Tensor):
      h =     x + self._attention(self.attn_norm(x), start_pos, pad_lens)
      return (h + self._feed_forward(self.ffn_norm(h))).contiguous()
    return _run(x, start_pos, pad_lens)
...
class TransformerBlock(FFNBlock):
  ...
  def _attention(self, x:Tensor, start_pos:int|UOp, pad_lens:Tensor) -> Tensor:
    ...
    mask = make_attn_mask(self.positions, pad_lens, start_pos, T, x.dtype)
    global _debug_first_layer
    _debug_first_layer = self._layer_idx == 0
  ...
  def _init_state(self, x:Tensor):
    if not hasattr(self, "cache_kv"):
      ...
      self.positions = precompute_positions(self.config.max_context, device=x.device)
...
class Transformer:
  def __init__(self, config:TransformerConfig):
    ...
    for i, blk in enumerate(self.blk): blk._layer_idx = i
  ...
  def forward(self, tokens:Tensor, start_pos:int|UOp, temperature:Tensor, pad_lens:Tensor) -> Tensor:
    x = self.token_embd(tokens).float()                   # (B, T, D)
    for block in self.blk: x = block(x, start_pos, pad_lens)
    logits = self.output(self.output_norm(x))[:, -1, :]
    # Gumbel-max trick: argmax(logits/temp - log(-log(uniform))) is equivalent to sampling from softmax(logits/temp)
    return (logits / temperature.maximum(1e-12) - (Tensor.rand_like(logits).maximum(1e-12).log().neg()).log()).argmax(-1, keepdim=True)

  def __call__(self, tokens:Tensor, start_pos:int|UOp, temperature:Tensor, pad_lens:Tensor) -> Tensor:
    return (self.prefill_jit if resolve(tokens.shape[1] != 1) else self.rollout_jit)(tokens.contiguous(), start_pos, temperature, pad_lens)
  ...
  def generate(self, seqs:list[int]|list[list[int]], chunk_size:int=32, temperature:float=0.0):
    ...
    while len(rows[0]) < self.max_context:
      ...
      out = self(t[:, sp:sp+nt] if start_pos < max_seq_len or out is None else out, sp, temp, pad_lens).realize()
      start_pos += nt.val
